...
public interface ScriptDocValues<T> extends List<T> {
    ...
    public static class GeoPoints extends AbstractList<GeoPoint> implements ScriptDocValues<GeoPoint> {
        ...
        public double arcDistance(double lat, double lon) {
            ...
            return DistanceUnit.DEFAULT.fromMeters(GeoDistanceUtils.vincentyDistance(point.lat(), point.lon(), lat, lon));
        }
        ...
        public double arcDistanceInKm(double lat, double lon) {
            GeoPoint point = getValue();
            return DistanceUnit.KILOMETERS.fromMeters(GeoDistanceUtils.vincentyDistance(point.lat(), point.lon(), lat, lon));
        }
        ...
        public double arcDistanceInMiles(double lat, double lon) {
            GeoPoint point = getValue();
            return DistanceUnit.MILES.fromMeters(GeoDistanceUtils.vincentyDistance(point.lat(), point.lon(), lat, lon));
        }
        ...
        public double distance(double lat, double lon) {
            GeoPoint point = getValue();
            return DistanceUnit.DEFAULT.fromMeters(GeoDistanceUtils.haversin(point.lat(), point.lon(), lat, lon));
        }
        ...
        public double distanceInKm(double lat, double lon) {
            GeoPoint point = getValue();
            return DistanceUnit.KILOMETERS.fromMeters(GeoDistanceUtils.haversin(point.lat(), point.lon(), lat, lon));
        }
        ...
        public double distanceInMiles(double lat, double lon) {
            GeoPoint point = getValue();
            return DistanceUnit.MILES.fromMeters(GeoDistanceUtils.haversin(point.lat(), point.lon(), lat, lon));
        }
        ...
        public double geohashDistance(String geohash) {
            ...
            return DistanceUnit.DEFAULT.fromMeters(GeoDistanceUtils.haversin(point.lat(), point.lon(), p.lat(), p.lon()));
        }

        public double geohashDistanceInKm(String geohash) {
            GeoPoint point = getValue();
            GeoPoint p = new GeoPoint().resetFromGeoHash(geohash);
            return DistanceUnit.KILOMETERS.fromMeters(GeoDistanceUtils.haversin(point.lat(), point.lon(), p.lat(), p.lon()));
        }

        public double geohashDistanceInMiles(String geohash) {
            GeoPoint point = getValue();
            GeoPoint p = new GeoPoint().resetFromGeoHash(geohash);
            return DistanceUnit.MILES.fromMeters(GeoDistanceUtils.haversin(point.lat(), point.lon(), p.lat(), p.lon()));
        }
    }
}
