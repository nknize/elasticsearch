...
public class ElasticsearchGeoAssertions {
    ...
    private static boolean isInTolerance(Coordinate p, Coordinate expected, double tolerance) {
    /*
     * Compare result to expected, for test purposes
     */
        double dx = Math.abs(p.x - expected.x);
        double dy = Math.abs(p.y - expected.y);
    ...
    public static void assertEquals(Coordinate[] c1, Coordinate[] c2, double tolerance) {
        ...
        for (int i = 0; i < c2.length; i++) {
            assertEquals(c1[i], c2[i], tolerance);
        }
    }

    public static void assertEquals(LineString l1, LineString l2, double tolerance) {
        assertEquals(l1.getCoordinates(), l2.getCoordinates(), tolerance);
    }

    public static void assertEquals(MultiLineString l1, MultiLineString l2, double tolerance) {
        assertEquals(l1.getCoordinates(), l2.getCoordinates(), tolerance);
    }

    public static void assertEquals(Polygon p1, Polygon p2, double tolerance) {
        Assert.assertEquals(p1.getNumInteriorRing(), p2.getNumInteriorRing());

        assertEquals(p1.getExteriorRing(), p2.getExteriorRing(), tolerance);
        ...
        for (int i = 0; i < p1.getNumInteriorRing(); i++) {
            assertEquals(p1.getInteriorRingN(i), p2.getInteriorRingN(i), tolerance);
        }
    }

    public static void assertEquals(MultiPolygon p1, MultiPolygon p2, double tolerance) {
    ...
    public static void assertEquals(Geometry s1, Geometry s2, double tolerance) {
        if(s1 instanceof LineString && s2 instanceof LineString) {
            assertEquals((LineString) s1, (LineString) s2, tolerance);

        } else if (s1 instanceof Polygon && s2 instanceof Polygon) {
            assertEquals((Polygon) s1, (Polygon) s2, tolerance);
            ...
            assertEquals((MultiPolygon) s1, (MultiPolygon) s2, tolerance);

        } else if (s1 instanceof MultiLineString && s2 instanceof MultiLineString) {
            assertEquals((MultiLineString) s1, (MultiLineString) s2, tolerance);
    ...
    public static void assertEquals(JtsGeometry g1, JtsGeometry g2, double tolerance) {
        assertEquals(g1.getGeom(), g2.getGeom(), tolerance);
    }

    public static void assertEquals(ShapeCollection s1, ShapeCollection s2, double tolerance) {
        Assert.assertEquals(s1.size(), s2.size());
        for (int i = 0; i < s1.size(); i++) {
            assertEquals(s1.get(i), s2.get(i), tolerance);
        }
    }

    public static void assertEquals(Shape s1, Shape s2, double tolerance) {
        if(s1 instanceof JtsGeometry && s2 instanceof JtsGeometry) {
            assertEquals((JtsGeometry) s1, (JtsGeometry) s2, tolerance);
            ...
            assertEquals((ShapeCollection)s1, (ShapeCollection)s2, tolerance);
